...
static void delayus(unsigned int us){
    int delay_us_num = ((CPUCLK_FREQ/1000000)*us);
    delay_cycles(delay_us_num);
}


static void delayms(unsigned int nms)///延时函数
{
    delay_cycles((CPUCLK_FREQ/1000)*nms);
}
#else
static void delayus(unsigned int us)
{
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(endraw==0&&t<(len-1))
		{
			if(temp==0)
			{
				ILI9341_draw_char(x+8*t,y,' ',0,COLOR);
				continue;
			}else endraw=1; 
		 	 
		}
	 	ILI9341_draw_char(x+8*t,y,temp+48,0,COLOR); 
	}
} 
/*---------------------------------------------------------------
函数功能：显示实数
...
/*********************************************************/
void ILI9341_Draw_Fast_VLine_With_Grid(uint16_t x, uint16_t start_y, uint16_t end_y, uint8_t is_erase, uint16_t wave_color)
{
    uint16_t color;

    // 1. 一次性开启垂直窗口 (极大地节省了重复发指令的时间)
    ILI9341_address_set(x, start_y, x, end_y);

    // 2. 准备发送内存写入指令 (一般是 0x2C)
    ...
    for(uint16_t y = start_y; y <= end_y; y++)
    {
        ...
        // 直接发 16 位颜色数据！没有坐标废话！
        // 注意：函数名替换为你底层实际写数据的名字，比如：
        // ILI9341_Write_Data_16Bit(color); 
        // 或者拆分成两个 8位发送：
        ILI9341_wr_data8(color >> 8); 
        ILI9341_wr_data8(color & 0xFF);
    }
}
